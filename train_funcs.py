...
import pdb
import wandb
wandb.init(project='251b_Final_Project')
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for phase in ["train","val"]:
            ...
            for key in epoch_acc_dict.keys():
                ...
                if (phase == "train"):
                    wandb.log({"train_loss"+str(key):epoch_loss_dict[key].data})
                    wandb.log({"train_acc"+str(key):epoch_acc_dict[key].data})
                elif (phase == "val"):
                    wandb.log({"val_loss"+str(key):epoch_loss_dict[key].data})
                    wandb.log({"val_acc"+str(key):epoch_acc_dict[key].data})
